...
import {insertIncome, updateIncome} from '../../service/IncomeService'
...
function IncomeEdit(props) {
    ...
    const handleSubmit = e => {
        ...
        props.form.validateFields((err, fieldsValue) => {
            ...
            if (id !== undefined) {
                updateIncome(id, fieldsValue).then(res => {
                    if (!res.success) {
                ...
                insertIncome(fieldsValue).then(res => {
                    if (!res.success) {
                        message.error(res.msg)
                        return
                    }
    ...
    return (
        <div>
            <Card title="收入明细编辑" extra={
                <Button type="primary" onClick={() => props.history.push("/income/")}>返回</Button>
            }>
                <Form onSubmit={e => handleSubmit(e)}>
                    <Form.Item label="收入类型">
                        {getFieldDecorator('type', {
                            ...
                            <Select style={{width: 1000}} allowClear placeholder="请选择类型" filterOption={false}>
                                {select.map(e => <Option value={e.id} key={e.id}>{e.type}</Option>)}
                            </Select>
                        )}
                    </Form.Item>
                    <Form.Item label="收入日期">
